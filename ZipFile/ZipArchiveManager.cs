...
namespace ZipFile
{
    class ZipArchiveManager
    {
        ...
        public static async Task<List<ZipArchiveEntry>> GetFileZip(StorageFile zipFile)
        {
            List<ZipArchiveEntry> list = new List<ZipArchiveEntry>();
            var extension = zipFile.FileType;
            if (zipFile == null || !extension.Equals(".zip", StringComparison.CurrentCultureIgnoreCase))
            {
                throw new ArgumentException("Invalid argument..." + extension);
            }
            Stream zipMemoryStream = await zipFile.OpenStreamForReadAsync();
            ...
            using (ZipArchive zipArchive = new ZipArchive(zipMemoryStream, ZipArchiveMode.Read))
            {
                list = zipArchive.Entries.ToList();
